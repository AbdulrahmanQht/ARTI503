...
class ParallelProfiler:
    ...
    def analyze_scalability(self,
                            ...
                            step: int = 100) -> Dict:
        """
        Tests scalability with HEAVIER loads to show real speedup.
        """
        print(f"\n{'=' * 80}")
        ...
        job_counts = [50, 100, 200, 500]  # Removed tiny counts
        batch_grid_size = 300  # INCREASED from 50 to 300
        print(f"  Using Grid Size: {batch_grid_size}x{batch_grid_size}")
        ...
        for num_jobs in job_counts:
            jobs = []
            for i in range(num_jobs):
                # Ensure start/goal are far apart for meaningful work
                job_start = (0, 0)
        ...
        for size in sizes:
            ...
            # Test Strategy 3
            # Cap splits at 50 to prevent Windows 63-handle crash
            raw_splits = size // 150
            ...
            print(f"    Region-Based:  {time_reg:.4f}s (using {splits} splits)")
    ...
    def profile_lock_contention(self, grid: np.ndarray,
        ...
        # Strategy 1 (Batch)
        print("\n[Strategy 1: Batch Processing]")
        batch_grid = create_random_grid(100, 100, 0.2)  # Increased from 50
        jobs = []
        for i in range(num_batch_jobs):
            job_start = (0, 0)
            job_goal = (99, 99)
    ...
    def run_comprehensive_profile(self, grid_size: int = 500):
        ...
        print(f"\nSetting up {grid_size}x{grid_size} grid...")
        ...
        all_results['bi'] = self.profile_strategy(
            "Strategy 2: Bidirectional",
            run_bidirectional_parallel,
            grid, start, goal
        )

        splits = min(50, max(2, grid_size // 50))
        all_results['reg'] = self.profile_strategy(
            "Strategy 3: Region-Based",
            run_region_parallel,
            grid, start, goal,
            splits=splits
...
def run_parallel_profile_mode():
    ...
    print("  3: Scalability Analysis (Increased Loads)")
    ...
    if choice == '1':
        ...
        grid[start] = 0
        grid[goal] = 0
        profiler.profile_batch_strategy(num_jobs=20, grid_size=300)
        profiler.profile_strategy("Bidirectional", run_bidirectional_parallel, grid, start, goal)
        profiler.profile_strategy("Region-Based", run_region_parallel, grid, start, goal, splits=10)
    elif choice == '3':
